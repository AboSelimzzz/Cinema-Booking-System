...
void mainmainmenu()
{
	...
	cout << "\033[1;31m\n\t\t\tPress anything\033[0m \033[1;33mto start\033[0m";
	cout << "\n\t\t\t";
	cin >> anything;
	system("CLS");
}
...
void intializer()
{
	...
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 5; j++) {
			...
			if (j == 0)
				movies[i].screen_info.hall.reseved_seats[j] = 99;
			else if (j == 1)
				movies[i].screen_info.hall.reseved_seats[j] = 99;
			else if (j == 2)
				movies[i].screen_info.hall.reseved_seats[j] = 98;
			else if (j == 3)
